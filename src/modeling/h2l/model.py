...
from modeling.spec_norm.normalization import SpectralNorm
...
class Discriminator(nn.Module):
    def __init__(self,input_size,in_channels=3):
        super().__init__()
        self.last_channels = (config.high_res // config.low_res) if input_size == config.high_res else (config.high_res // config.low_res)//2
